...
**Namespaces are never created as a side effect.** `backup_secrets/main.go` and `arangodb-operator` both probe the `namespace-bootstrap` stack and deploy to its `appNamespace` export (`prod`). The stack also creates shared namespace `operators` for other operators. Missing bootstrap fails at preview; `configure-backup-secrets` rejects an explicit namespace that disagrees with `appNamespace` before Pulumi work.
