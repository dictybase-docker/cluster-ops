...
| `reset-cluster` refuses: `no recovery source on stack` | Recovery source not configured before the reset | Run `just postgres configure-source --source-stack <name>` first — resetting without it would bootstrap empty |
