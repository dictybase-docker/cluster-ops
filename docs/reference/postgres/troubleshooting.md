...
| `restore-logical` aborts with a raw `pulumi`, `kubectl`, or `gcloud storage` error during its pre-flight | The state probe fails closed by design: only "config key not found", `NotFound` on the Cluster, and "matched no objects" on the backup prefix are treated as clean-target answers. Any other failure (expired credentials, wrong kube context, network) is propagated rather than read as "nothing there" | Fix the underlying access problem and re-run — the abort happened before any mutation, so the target is unchanged ([details](logical-import.md#behavior--restore-logical)) |
