...
readiness, data PVC, Service port, auth Secret, and one authenticated `PING`.
Changes nothing; safe to re-run at any time.
...
6. **Auth handshake** — `redis-cli -a <password from Secret> --no-auth-warning PING` inside the pod returns `PONG`

The handshake step is skipped when no ready pod was found (steps 2 and 6 then
...
| `--namespace` / `-n` | No | `prod` | |
| `--secret` / `-e` | No | `redis-auth` | Password is read from key `password` |
