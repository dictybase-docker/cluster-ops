...
func verifyTopLevelSpec(t *testing.T, spec map[string]any) {
	t.Helper()
	assert.Equal(t, defaultMode, spec["mode"])
	assert.Equal(t, defaultEnvironment, spec["environment"])
	assert.Equal(t, "arangodb:3.12.11", spec["image"])
	...
	require.True(t, ok)
	passwordSecretNames, ok := bootstrap["passwordSecretNames"].(map[string]any)
	require.True(t, ok)
	assert.Equal(t, "arangodb-pass", passwordSecretNames["root"])

	externalAccess, ok := spec["externalAccess"].(map[string]any)
	require.True(t, ok)
	assert.Equal(t, "None", externalAccess["type"])
}

func verifyAgentSpec(t *testing.T, spec map[string]any) {
	t.Helper()
	agents, ok := spec["agents"].(map[string]any)
	require.True(t, ok)
	...
	tolerations, ok := agents["tolerations"].([]map[string]any)
	require.True(t, ok)
	assert.Len(t, tolerations, 1)
	assert.Equal(t, tolerationKey, tolerations[0]["key"])

	resources, ok := agents["resources"].(map[string]any)
	require.True(t, ok)
	...
	vct, ok := agents["volumeClaimTemplate"].(map[string]any)
	require.True(t, ok)
...
func verifyDBServerSpec(t *testing.T, spec map[string]any) {
	t.Helper()
	dbservers, ok := spec["dbservers"].(map[string]any)
	require.True(t, ok)
	...
	resources, ok := dbservers["resources"].(map[string]any)
	require.True(t, ok)
	requests, ok := resources["requests"].(map[string]any)
	require.True(t, ok)
	...
	vct, ok := dbservers["volumeClaimTemplate"].(map[string]any)
	require.True(t, ok)
	vctSpec, ok := vct["spec"].(map[string]any)
	require.True(t, ok)
...
func verifyCoordinatorSpec(t *testing.T, spec map[string]any) {
	t.Helper()
	coordinators, ok := spec["coordinators"].(map[string]any)
	require.True(t, ok)
	assert.Equal(t, 3, coordinators["count"])
	assert.Nil(t, coordinators["volumeClaimTemplate"])
	assert.NotNil(t, coordinators["affinity"])

	resources, ok := coordinators["resources"].(map[string]any)
	require.True(t, ok)
	requests, ok := resources["requests"].(map[string]any)
	require.True(t, ok)
