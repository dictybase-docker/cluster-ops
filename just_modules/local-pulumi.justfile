...
[group('pulumi-local')]
[no-cd]
new-stack folder stack="local" pass_entry="pulumi/local-passphrase":
    #!/usr/bin/env bash
    set -euo pipefail
...
[group('pulumi-local')]
[no-cd]
new-stack-from folder stack="local" from-stack="experiments" pass_entry="pulumi/local-passphrase":
    #!/usr/bin/env bash
    set -euo pipefail
