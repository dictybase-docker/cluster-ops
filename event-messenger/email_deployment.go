...
func (eme *EventMessengerEmail) CreateDeployment(ctx *pulumi.Context) (*appsv1.Deployment, error) {
	deployment, err := appsv1.NewDeployment(ctx, eme.Config.Email.Name, &appsv1.DeploymentArgs{
		Metadata: eme.CreateDeploymentMetadata(),
		Spec:     eme.CreateDeploymentSpec(),
	})
	if err != nil {
		return nil, fmt.Errorf("error creating %s deployment: %w", eme.Config.Email.Name, err)
	}
